...
function Scene() {
  ...
  useFrame((state) => {
    ...
    lerp(rainbow.current.material, 'emissiveIntensity', isPrismHit ? 0.4 : 0, 0.1)
    spot.current.intensity = rainbow.current.material.emissiveIntensity
    // Animate ambience
    lerp(ambient.current, 'intensity', 0, 0.00025)
  })
  ...
  return (
    <>
      ...
      <Center top bottom position={[0, 2, 0]}>
        <Text3D size={0.3} letterSpacing={-0.03} height={0.05} font="/fonts/Inter_Bold.json">
          que pasa con esta fuente
